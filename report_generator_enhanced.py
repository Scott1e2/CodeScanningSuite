...
# report_generator.py - Enhanced Vulnerability Reporting, Compliance Mapping, and Drift Tracking for DAST/SAST Suite

import json
import os
import matplotlib.pyplot as plt

# Define scoring criteria, compliance mappings, and use case tracking
SEVERITY_SCORES = {
    "critical": 10,
    "high": 7,
    "medium": 5,
...
COMPLIANCE_MAPPING = {
    "sql_injection": ["OWASP A1: Injection", "PCI-DSS 6.5.1"],
    "xss": ["OWASP A7: Cross-Site Scripting (XSS)", "PCI-DSS 6.5.7"],
    "csrf_token_missing": ["OWASP A8: Cross-Site Request Forgery (CSRF)", "GDPR Art. 32"],
    "xxe": ["OWASP A5: XML External Entities", "CWE-611"],
    "cors_misconfig": ["CWE-942: Permissive CORS", "NIST 800-53 AC-4"]
}
...
# Generate report with risk scores, compliance mappings, and use case tracking
    ...
    if output_format == "text":
        with open("dast_sast_report.txt", "w") as report_file:
            ...
            for vuln in vulnerabilities:
                ...
                report_file.write(f"Compliance: {', '.join(vuln['compliance_mapping'])}\n")
                report_file.write(f"Mapped to Use Case: {vuln.get('use_case', 'general_security')}\n")
